...
document.addEventListener('DOMContentLoaded', function () {
    ...
    inputCNPJ.addEventListener('input', function () {
        ...
        if (cnpjValue.length > 2) {
            formattedCNPJ += '.' + cnpjValue.substring(2, 5);
        }

        if (cnpjValue.length > 5) {
            formattedCNPJ += '.' + cnpjValue.substring(5, 8);
        }

        if (cnpjValue.length > 8) {
            formattedCNPJ += '/' + cnpjValue.substring(8, 12);
        }

        if (cnpjValue.length > 12) {
            formattedCNPJ += '-' + cnpjValue.substring(12, 14);
        }
